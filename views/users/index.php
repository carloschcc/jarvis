                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <div class="input-group-append">
                                                    <button type="button" class="btn btn-outline-secondary btn-sm" onclick="togglePasswordVisibility('createPassword')" style="padding: 4px 6px; width: 32px; min-width: 32px;">
                                                        <i class="fas fa-eye" style="font-size: 11px;"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="generateRandomPassword()" style="padding: 4px 6px; width: 32px; min-width: 32px;">
                                                        <i class="fas fa-dice" style="font-size: 11px;"></i>
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="form-section" style="background: linear-gradient(135deg, #f8f9fa, #e9ecef); border: 2px solid #007bff; border-radius: 8px;">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div id="groupsList" class="groups-container" style="max-height: 200px; overflow-y: auto; border: 1px solid #dee2e6; border-radius: 6px; padding: 10px; background: #ffffff;">
                                                ...
                                                <!-- Botão para carregar grupos do AD -->
...
function loadMoreGroups() {
    ...
    // Mostrar loading
    button.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Carregando grupos do AD...';
    button.disabled = true;
    
    // Obter token CSRF
    const csrfToken = document.querySelector('input[name="csrf_token"]').value;
    
    // Fazer requisição AJAX real para buscar grupos do AD
    fetch('index.php?page=groups&action=getGroups', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        // Grupos já existentes (para evitar duplicatas)
        const existingGroups = new Set();
        document.querySelectorAll('#groupsList .form-check-input').forEach(input => {
            existingGroups.add(input.value.toLowerCase());
        });
        
        let addedCount = 0;
        
        // Adicionar grupos do AD (excluir Domain Users que já está)
        data.groups.forEach(group => {
            // Pular se já existe ou é Domain Users
            if (existingGroups.has(group.name.toLowerCase()) || group.name.toLowerCase() === 'domain users') {
                return;
            }
        ...
        if (addedCount > 0) {
            showNotification(`✅ ${addedCount} grupos do Active Directory carregados com sucesso!`, 'success');
        } else {
            showNotification('ℹ️ Nenhum grupo adicional encontrado no AD', 'info');
        }
    })
    .catch(error => {
        console.error('Erro ao carregar grupos do AD:', error);
        
        // Restaurar botão em caso de erro
        button.innerHTML = originalText;
        button.disabled = false;
        
        // Mostrar erro e fallback para grupos padrão
        showNotification('⚠️ Erro ao conectar com AD. Usando grupos padrão.', 'warning');
        
        // Fallback: carregar alguns grupos padrão comuns em AD
        loadFallbackGroups();
    });
}

// Função de fallback para grupos padrão quando AD não está disponível
function loadFallbackGroups() {
    ...
    const groupsContainer = document.getElementById('groupsList');
    const loadButton = document.querySelector('#groupsList button');
    
    fallbackGroups.forEach(group => {
        const groupHtml = `
            <div class="form-check group-item" style="margin-bottom: 8px; padding: 8px; background: #fff3cd; border-radius: 4px; border: 1px solid #ffeaa7;">
                <input class="form-check-input" type="checkbox" value="${group.name}" id="group_${group.id}">
                <label class="form-check-label" for="group_${group.id}" style="color: #856404; display: flex; align-items: center;">
                    <i class="${group.icon}" style="margin-right: 6px; color: ${group.color};"></i>
                    ${group.name} (Padrão)
                </label>
                <small class="d-block text-muted" style="margin-left: 20px; font-size: 0.75rem;">${group.description}</small>
        ...
        loadButton.parentElement.insertAdjacentHTML('beforebegin', groupHtml);
    });
}
